...
| Integration | `internal/server/`, `internal/client/`, `cmd/client/`, or root | Real server on `:0`, real WebSocket client, play through a full game. WebSocket upgrade, message framing, close frames, and concurrent clients use `httptest.Server` + `websocket.Dial`. Exercises the same code path as production. |
